document.addEventListener("DOMContentLoaded", function () {
    ...
    const departamentoResidencia = document.getElementById(
    ...
    //Inicio Validaciones para el formulario y demas de ESTABLECIMIENTO
    // Cargar ciudades dinámicamente según el departamento en establecimiento
    const deptoEstablecimiento = document.getElementById(
        "departamento_establecimiento"
    );
    const ciudadEstablecimiento = document.getElementById(
        "ciudad_establecimiento"
    );

    if (deptoEstablecimiento && ciudadEstablecimiento) {
        deptoEstablecimiento.addEventListener("change", function () {
            const departamentoID = this.value;

            ciudadEstablecimiento.innerHTML =
                '<option value="">Seleccione una ciudad</option>';
            ciudadEstablecimiento.disabled = true;
                ...
                fetch(`/get-ciudades?departamento_id=${departamentoID}`)
                    ...
                    .then((data) => {
                        data.forEach((ciudad) => {
                            ...
                            ciudadEstablecimiento.appendChild(option);
                        });
                        ciudadEstablecimiento.disabled = false;
                    })
                    .catch((error) => {
    ...
    //Fin Validaciones para el formulario y demas de ESTABLECIMIENTO
    ...
    camposMayusculas.forEach((nombreCampo) => {
        ...
        if (input) {
            ...
            input.addEventListener("keypress", function (e) {
                ...
                if (
                    !/^[A-ZÁÉÍÓÚÑÜ0-9 .,-]$|Backspace|Delete|ArrowLeft|ArrowRight|Tab$/i.test(
                        key
                    )
                ) {
                    e.preventDefault();
    ...
    const nitEstablecimiento = document.querySelector(
        'input[name="nit_establecimiento"]'
    );
    ...
    soloLetras.forEach((id) => {
        ...
        if (input) {
            ...
            input.addEventListener("keypress", function (e) {
                ...
                if (
                    !/^[A-ZÁÉÍÓÚÑ ]$|Backspace|Delete|ArrowLeft|ArrowRight|Tab$/i.test(
                        key
                    )
                ) {
                    e.preventDefault();
...
$(document).ready(function () {
    $("#departamento_nacimiento").on("change", function () {
        ...
        if (departamentoID) {
            ...
            $ciudadSelect
                .empty()
                .append('<option value="">Seleccione una ciudad</option>')
                .prop("disabled", true);
        }
    });
});
